/*
 * This program takes colours from the 24-bit true colour depth and produces the closest match from the 8-bit palette.
 ...
 */
/*
 * Attempt matching greys only if the rgb components are close to each other. The worst case is 47,0,0. This should...
 * match to 0,0,0 because 95,0,0 is further away. 48,0,0 will go up to 95,0,0. Hence, if the max difference between...
 * rgb components is more than 47, don't bother matching greys.
 */
int calcrgbdiff(int rgb[])
{
    int diff;
...
{
    /*
     * Since the  <=  operator is used in the condition, the brighter colour--which is produced later in the loop...
     * because that is how it is written--overwrites the darker colour even though they have the same difference in...
     * rgb components.
     */
...
/* TODO:
 * Once above 95, the jump in rgb component value is 40. So if the max difference between the rgb components is more...
 * than half of 40, don't bother finding the closest grey match because there will necessarily be a colour match that...
 * is closer. */
...
void loopbasecols(int rgb[], int bmatch[])
{
    ...
    for (b = 0; b < 2 && i < 2; ++b) {    // second condition is just to avoid an infinite loop
        ...
        if (b == 1) {            // once the loop has run through as if the second condition were not there,...
            colours[1] = 255;    // run through it again but using 255 instead of 128
            b = -1;
            ++i;
        }
    }
