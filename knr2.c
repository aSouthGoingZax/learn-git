...
{
    int i, j;
    int c = 97;    // value of  a  in character set
 ...
 */
/* Exercise 2-9
 * x &= (x - 1);    // sets the rightmost bit of  x  that is 1 to 0
...
int countactivebits()
{
    ...
    for (b = 0; x != 0; x &= x - 1)    // faster version of the loop above
    {
        b++;
    }
...
int main()
{
    ...
    a = typeconversiondemo();
    a = incrdecrdemo();
    a = deletecommonchars();
    a = checksharedstringchars();
    a = bitwiseoperatordemo();
    a = grabandplacebits();
    a = invertspecificbits();
    a = rightrotatebits();
    a = countactivebits();
    a = inlineifdemo();
